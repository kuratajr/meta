...
function renderUI(data) {
    ...
    document.getElementById('stat-active-groups').innerText = groupsData.length;
...
function renderGroups(data) {
    ...
    if (!gBody) return;
    gBody.innerHTML = (data.groups || []).filter(g => !currentSearch || g.config.toLowerCase().includes(currentSearch)).map(g => `<tr>
...
function renderTemplates(data) {
    ...
    if (!tBody) return;
    tBody.innerHTML = data.templates.filter(t => !currentSearch || t.toLowerCase().includes(currentSearch)).map(t => `<tr>
...
function renderConfigs(data) {
    ...
    cBody.innerHTML = allConfigs.filter(c => !currentSearch || c.toLowerCase().includes(currentSearch)).map(c => `<tr>
...
async function refreshStatusDots() {
    ...
    for (let i = 0; i < total; i += batchSize) {
        promises.push(fetch(`/api/batch-check-nodes?token=${TOKEN}&offset=${i}&limit=${batchSize}&_=${Date.now()}`).then(r => r.json()).catch(() => ({})));
    }
    try {
        ...
        for (const node of hostnames) {
            ...
            if (prevStatus !== undefined && newStatus !== prevStatus) {
                ...
                fetch(`/api/record-log?token=${TOKEN}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ msg, node }) }).catch(() => { });
            }
